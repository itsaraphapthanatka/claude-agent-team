---
name: agent-team
description: "Bootstrap or extend a full software-company agent team (PM, architect, UX, backend/mobile/web devs, SDET, QA testers, code reviewer, bug triager, security, devops, tech writer) plus pipeline commands (/feature, /fix, /test-all, …) in the current project, grounded in that project's real code. Use when the user wants an agent team, dev team, QA team, tester team, or says สร้างทีม / ทีม agent / ทีม dev / ทีม tester for a project."
