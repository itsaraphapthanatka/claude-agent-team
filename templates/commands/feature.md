...
1. **Product.** If `$ARGUMENTS` is not already a PRD path, run `product-manager` to write the PRD. Then run `architect` on that PRD to write `{{DOCS_DIR}}/design/DESIGN-<slug>.md`, and in parallel `ux-designer` if screens change, then `ui-designer` for the component spec once the UX spec exists (skip either role if it is not in this project's team).
